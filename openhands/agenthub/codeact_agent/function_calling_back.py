    ...
    if assistant_msg.tool_calls:
        ...
        for i, tool_call in enumerate(assistant_msg.tool_calls):
            ...
            try:
                ...
                if 'fields' in all_keys and llm is not None:
                    tools = [CmdRunTool, FinishTool, IPythonTool, StrReplaceEditorTool, RegressionTool, LLMBasedFileEditTool]
                    # tool_doc = [tool for tool in tools if tool['function']['name'] == tool_call.function.name][0]
                    ...
                    tools = [
                        ...
                        {
                            'name': 'execute_bash',
                            'doc': {'command': 'actual command (required)'}
                        },
                        {
                            'name': 'execute_ipython_cell',
                            'doc': {'code': 'actual code (required)'}
                        },
                        {
                            'name': 'str_replace_editor',
                            'doc': {
                                'command': 'actual command (required)', 
                                'path': 'actual path (required)',
                                'file_text': 'actual file text (optional)',
                                'old_str': 'actual old string (optional)',
                                'new_str': 'actual new string (optional)',
                                'insert_line': 'actual insert line (optional)',
                                'view_range': 'actual view range (optional)'
                            }
                        },
                        {
                            'name': 'edit_file',
                            'doc': {
                                'path': 'actual path (required)',
                                'content': 'actual content (required)',
                                'start': 'actual start (optional)',
                                'end': 'actual end (optional)'
                            }
                        }
                    ]
                    tool_doc = [tool['doc'] for tool in tools if tool['name'] == tool_call.function.name][0]
                    tool_doc = json.dumps(tool_doc, indent=4)
                    retry_count = 0
                    while True: 
                        # import pdb; pdb.set_trace()
                        messages=[
                            {
                                'role': 'user',
                                'content': (
                                    f'## Tool Parameters Example: {tool_doc}\n\n'
                                    f'Please assist me in parsing the following parameters for the function `{tool_call.function.name}` into the correct format: {raw_arguments}\n\n'
                                    'Kindly return the corrected arguments in JSON format within the code block below for easy parsing:\n'
                                    '```json\n```'
                                )
                            }
                        ]
                        ...
                        if match:
                            ...
                            retry_count += 1
                            if retry_count > 5:
                                raise RuntimeError('Failed to parse tool call arguments')
                else:
